...
func NewLocal(basePath string, maxSize int) (*Local, error) {
	...
	return &Local{basePath: p}, err
}

// Save the contents of the Reader to the given path
...
func (l *Local) Save(path string, contents io.Reader) error {
	...
	// ensure that we are not writing greater than mzx bytes
...
// Get the file at the given path and returns a Reader
// the calling functions is responsible for closing the reader
